...
module.exports = class Netcode {
  ...
  remoteInput(tick, dir) {
    ...
    // Reenvio tras un swap de socket: el lobby re-manda los ultimos inputs por
    // si alguno se fue con el socket muerto. Si este ya esta registrado igual,
    // no hay nada que reconciliar — rebobinar daria el mismo estado y solo
    // inflaria el contador de rollbacks que muestra la UI.
    const slot = this.inputs.get(tick)
    if (slot !== undefined && slot[other] === dir) return true
