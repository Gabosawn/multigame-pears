...
// Una arena llega de dos formas: como definicion (con `preset`, la forma chica
// del registro) o YA CONSTRUIDA (con `walls`), que es lo que viaja en MATCH
// porque el anfitrion manda su arena entera. Si vienen paredes explicitas van a
// misa: regenerar desde el preset era EL bug — el objeto construido no llevaba
// `preset`, asi que el invitado caia al default (vacio) y jugaba la misma arena
// SIN paredes. Mismo id en las dos puntas, otra simulacion: desincronizacion
// garantizada en cuanto una serpiente o la comida tocaban un muro que el otro
// no tenia. Las celdas de afuera del tablero se filtran porque las paredes
// pueden venir de la red, y una celda absurda no puede romper la simulacion.
function build({ id, name, w, h, preset, tickRate, walls }) {
  const total = w * h
  const given = Array.isArray(walls)
    ? walls.filter((cell) => Number.isInteger(cell) && cell >= 0 && cell < total)
    : null
  const gen = PRESETS[preset] || PRESETS.vacio
  return { id, name, w, h, tickRate, preset, walls: given !== null ? given : gen(w, h) }
}
...
// Las de fabrica mas lo que haya llegado por el canal de contenido, ordenadas
// por id para que la rotacion no dependa del orden en que llegaron.
function catalog(extra = []) {
  const seen = new Set()
  const list = []
  for (const def of BUILTIN.concat(extra)) {
    if (!def || typeof def.id !== 'string' || seen.has(def.id)) continue
    seen.add(def.id)
    try {
      list.push(build(def))
    } catch {
      // definicion rota que llego por la red: ignorarla en vez de morir
    }
  }
...
module.exports = {
  ...
  get: (id, extra = []) => {
    const list = catalog(extra)
    return list.find((arena) => arena.id === id) || list[0]
  },
  // rota entre las disponibles: cada revancha estrena tablero, y una arena
  // recien llegada entra en la rotacion sin reiniciar nada
  pick: (n, extra = []) => {
    const list = catalog(extra)
