...
// Cuanto atras se mira para comparar. No es solo por el desfase de relojes: mi
// hash del tick T no incluye un input del rival sellado en T-2 que todavia esta
// en vuelo, asi que comparar demasiado cerca del presente da falsos positivos.
// 24 ticks son ~2s a 12Hz, comodo para cualquier latencia razonable.
const SYNC_LAG = 24

// Cuantas comparaciones seguidas tienen que fallar antes de resincronizar.
//
// Un input en vuelo hace fallar UNA comparacion y se resuelve solo en la
// siguiente. Una divergencia de verdad es permanente y falla todas. Sin esta
// confirmacion, un paquete que tarda un poco mas dispara un snapshot completo y
// un reajuste de reloj — o sea un tiron visible — sin que nada estuviera mal.
//
// Sobre un stream ordenado y confiable una divergencia real solo puede venir de
// una reconexion (los INPUT son efimeros y se descartan durante el corte), asi
// que detectarla en dos rondas en vez de una no cuesta nada en la practica.
const SYNC_CONFIRM = 2
...
module.exports = {
  ...
  //
  // `arena` llega YA CONSTRUIDA y es la misma en las dos puntas porque el
  // anfitrion la manda entera al anunciar la partida. Ojo: no puede depender de
  // nada local (tamano de la terminal, contenido que tenga cada uno) o las
  // simulaciones divergen.
  init({ first, seed, arena: given }) {
    const arena = given || arenas.get('vacio')
    ...
    return {
      ...
      lastSyncSent: -1,
      syncMisses: 0, // comparaciones de hash fallidas seguidas
  ...
  onPeerMsg(state, msg) {
    ...
    if (msg.t === protocol.T.SYNC) {
      ...
      if (mine === null) return { state } // sin historia en ese tick: no opinamos
      ...
      // Confirmar antes de actuar: un input en vuelo hace fallar una sola
      // comparacion, una divergencia de verdad falla todas.
      state.syncMisses += 1
      if (state.syncMisses < SYNC_CONFIRM) return { state }
      state.syncMisses = 0
