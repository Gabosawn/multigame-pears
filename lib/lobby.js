...
//
// STATE esta aca por una razon que no es obvia: es el snapshot con el que un peer
// resincroniza snake, y adopt() reajusta el reloj al tick del snapshot. Reenviar
// uno viejo REBOBINA la partida. Perderlo, en cambio, es inofensivo: el proximo
// chequeo de hash detecta la diferencia y pide otro. Asimetria clara, y gana
// perderlo.
...
// Los INPUT tambien se reenvian tras un swap, pero por un canal propio y sin
// numero: van sellados con su tick absoluto y el netcode los aplica idempotente
// (re-aplicar el mismo input re-simula al mismo estado), asi que un duplicado
// es gratis. Perderlos, en cambio, no: el dedup de hyperswarm puede matar el
// socket perdedor SEGUNDOS despues de emparejar — medido: ~6s en loopback — y
// un input que viajaba ahi separaba las dos simulaciones para siempre, hasta
// que el chequeo de hash pedia un snapshot completo. Un tiron visible por un
// swap que tendria que ser invisible. Lo demas sigue sin reenviarse: un PING
// viejo miente sobre la latencia, un SYNC viejo no le sirve a nadie y un STATE
// viejo rebobina la partida.
const INPUT_REPLAY_MAX = 32
...
module.exports = class Lobby extends ReadyResource {
  ...
  _onconnection(conn, via) {
    ...
    // rival nuevo: los contadores arrancan de cero en las dos puntas. El outbox
    // tambien: lo encolado era para el rival anterior y lleva numeros de esa
    // conversacion — entregarselo al nuevo le adelantaria el contador de vistos
    // y le haria descartar los mensajes reales que vengan despues.
    if (!reconnect) {
      this._seq = 0
      this._seen = 0
      this._replay = []
  ...
  _attach(conn, via) {
    ...
    // el socket cambia pero la conversacion sigue: hay que recuperar lo que el
    // anterior pudo haberse llevado. Da igual si el reemplazo llego con el
    // viejo todavia puesto (swap por dedup) o despues de perderlo (reconexion
    // dentro del margen): en los dos casos lo escrito sobre el moribundo puede
    // no haber salido nunca.
    const resumed = old !== null || this._lostKey !== null
    ...
    // El receptor descarta por numero lo que ya vio, asi que reenviar es
    // seguro. El orden importa: el replay (lo ya enviado, numeros mas viejos)
    // va ANTES que el outbox (lo encolado durante el corte, numeros mas
    // nuevos) — al reves, el contador de vistos del receptor se adelantaria y
    // descartaria como repetido lo reenviado que nunca le llego.
    if (resumed) {
    ...
    // y los inputs recientes, por si alguno viajaba en el socket muerto: llegan
    // sellados con su tick y el netcode reconcilia o ignora los repetidos
    if (resumed) {
      for (const data of this._inputReplay) this.peer.pipe.write(data)
    }
  ...
  _onframe(data) {
    ...
    // la partida nueva que anuncia el rival tambien deja viejos NUESTROS
    // inputs: sus ticks son de la partida anterior (esto corre despues del
    // descarte por numero, asi que un MATCH reenviado no borra nada)
    if (msg.t === protocol.T.MATCH) this._inputReplay = []
  ...
  send(msg) {
    ...
    // una partida nueva resetea los ticks: los inputs de la anterior, sellados
    // con ticks de otra linea de tiempo, no pueden reaparecer en esta
    if (msg.t === protocol.T.MATCH) this._inputReplay = []
    ...
    if (msg.t === protocol.T.INPUT) {
      this._inputReplay.push(data)
      if (this._inputReplay.length > INPUT_REPLAY_MAX) this._inputReplay.shift()
    }
    ...
    // volver. Lo que sigue siendo cierto mas tarde: un movimiento por turnos
    // si; un hash de sincronia o un ping solo tienen sentido en el momento —
    // entregarlos despues miente. Los INPUT son el caso especial: van sellados
    // con su tick absoluto, asi que quedaron en su propio ring de arriba y el
    // reenganche los entrega — el netcode rebobina hasta ese tick y la partida
    // sigue como si el corte no hubiera existido.
    if (this._lostKey !== null && msg.t === protocol.T.INPUT) return true
