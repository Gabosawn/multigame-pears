...
module.exports = class UI {
  constructor({ version, content = null, prefs = null, bluetooth = true }) {
  ...
  _roomKey(key) {
    ...
    // El nombre viaja igual en las dos puntas porque el topic sale del UTF-8 de
    // la cadena, asi que acentos y ñ son bienvenidos.
  ...
  _hostMatch() {
    ...
    if (this.game.realtime) {
      // freshArena() devuelve la DEFINICION cruda del registro (preset, sin
      // walls): hay que construirla antes de jugarla, igual que hace catalog()
      // con la rotacion. Usarla sin construir dejaba al anfitrion sin paredes
      // (walls undefined) mientras el rival si las generaba: la estrenaban en
      // dos arenas distintas con el mismo nombre.
      const fresh = this.content !== null ? this.content.freshArena() : null
      if (fresh !== null) {
        try {
          arena = arenas.build(fresh)
        } catch {
          arena = null // definicion rota que llego por la red: no rompe la revancha
        }
      }
      if (arena === null) arena = arenas.pick(this.matchNo, extra)
    }
  ...
  async _play() {
    ...
    this.lobby = new Lobby(this.game.id, this.room, { bluetooth: this.bluetooth })
