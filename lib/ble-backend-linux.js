...
class Central extends EventEmitter {
  ...
  disconnect(peripheral) {
    // En Linux NO se corta el radio: entre dos maquinas dual-role las DOS
    // direcciones comparten UN solo enlace fisico (ACL). Cuando ble-swarm
    // retira el canal redundante del dedup (los dos se marcaron a la vez) y
    // pide "desconectar", un Disconnect() de BlueZ tira el enlace ENTERO y
    // mata tambien al canal ganador: la partida conectaba ~2 segundos y se
    // caia, en loop. En iOS/Android cada rol maneja su enlace y esto no pasa;
    // aca el enlace huerfano se limpia solo cuando ambos extremos lo sueltan.
  }
}
